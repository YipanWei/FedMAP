...
_C.TRAIN.LOG_EVERY = 1   # 1=每一步都保存；10=每10步保存一次（更省 IO）
...
# =========================
_C.SEMANTIC = CN()
_C.SEMANTIC.ROOT = "."  # 项目根目录（包含 CLS_Exp/ 和 embeddings/）
_C.SEMANTIC.FORM = "attr"                             # "attr" or "desc"
_C.SEMANTIC.USE_ALL = False                           # True -> ALL, False -> only one
_C.SEMANTIC.IDX = 0                                   # 指定一条的 index（支持 -1 表示最后一条）
_C.SEMANTIC.MULTI_REDUCE = "mean_then_calc"

# （可选）如果你未来要做 pooling/multi-k 曲线，可以先占位
_C.SEMANTIC.POOLING = "mean"                           # 目前只有 mean
_C.SEMANTIC.K = 1                                      # 目前 USE_ALL/IDX 足够，但先留着不坏事

# =========================
# Injection / alignment mode
# =========================
_C.INJECT = CN()
_C.INJECT.MODE = "concat"                              # "concat" | "kl" | "mse"

# loss weights / temperature
_C.INJECT.LAMBDA_KL = 1.0                              # KL loss 权重（MODE=kl 时用）
_C.INJECT.LAMBDA_MSE = 0.5                             # MSE loss 权重（MODE=mse 时用）
_C.INJECT.T = 0.1                                      # KL 温度（MODE=kl 时用）
_C.INJECT.KL_ALPHA = 1.0                               # KL rongheqiangdu
# （可选）当 MODE=concat 且你拼接多条文本时，用什么分隔符
_C.INJECT.CONCAT_SEP = "; "                            # 用于把多行 LLM 扩展拼成一个字符串