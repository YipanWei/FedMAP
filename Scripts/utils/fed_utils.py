...
FOCOOP_KEYS_GLOBAL = ["prompt_learner.ctx_g", "prompt_learner.ctx_o"]
FOCOOP_KEY_LOCAL = "prompt_learner.ctx_l"

def _has_keys(sd, keys):
    return all(k in sd for k in keys)

def extract_focoop_parts(state_dict):
    """
    从完整 state_dict 里抽取 FOCoOp 相关的三块：
      - ctx_l (local)
      - ctx_g (global)
      - ctx_o (ood)
    """
    out = {}
    if FOCOOP_KEY_LOCAL in state_dict:
        out[FOCOOP_KEY_LOCAL] = state_dict[FOCOOP_KEY_LOCAL].detach().clone()
    for k in FOCOOP_KEYS_GLOBAL:
        if k in state_dict:
            out[k] = state_dict[k].detach().clone()
    return out

def overwrite_keys_(dst_state_dict, src_state_dict, keys):
    """
    用 src 中的 keys 覆盖 dst 中对应 keys（原地修改 dst）。
    """
    for k in keys:
...
def average_focoop_global_parts(local_full_state_dicts, idxs_users):
    """
    等权聚合：只平均 ctx_g / ctx_o
    local_full_state_dicts: dict[idx] -> model.state_dict() (完整或至少包含相关 key)
    """
    assert len(idxs_users) > 0
    # 取第一个作为模板
    first_sd = local_full_state_dicts[idxs_users[0]]
    assert _has_keys(first_sd, FOCOOP_KEYS_GLOBAL), f"Missing keys in local sd: {FOCOOP_KEYS_GLOBAL}"

    w_avg = {}
    for k in FOCOOP_KEYS_GLOBAL:
        w_avg[k] = first_sd[k].detach().clone().zero_()

    m = len(idxs_users)
    for idx in idxs_users:
        sd = local_full_state_dicts[idx]
        for k in FOCOOP_KEYS_GLOBAL:
            w_avg[k] += sd[k].detach()

    for k in FOCOOP_KEYS_GLOBAL:
        w_avg[k] = w_avg[k] / float(m)

    return w_avg

def build_global_eval_state(global_weights):
    """
    全局测试时没有某个具体 client 的 ctx_l。
    简化做法：令 ctx_l = ctx_g（这样 mixed ctx = ctx_g，避免依赖 client local）。
    """
    eval_sd = copy.deepcopy(global_weights)
    if "prompt_learner.ctx_g" in eval_sd and FOCOOP_KEY_LOCAL in eval_sd:
        eval_sd[FOCOOP_KEY_LOCAL] = eval_sd["prompt_learner.ctx_g"].detach().clone()
    return eval_sd
...
def save_acc_csv(para_dir, global_test_acc_dict, cfg, start):

    # 创建输出目录
    os.makedirs(para_dir, exist_ok=True)

    # start 是训练开始时记录的 time.time()
    ...
    acc_path = os.path.join(
        para_dir,
        f"acc_{start_time_str}_{duration}.csv"
    )
    ...
    with open(acc_path, 'w', newline='') as f:
        ...
        # 表头： domain, epoch_0, epoch_1, ...
        header = ["domain"] + [f"epoch_{i}" for i in range(cfg.OPTIM.ROUND)]
        writer.writerow(header)

        # 每个方法写一行
        for method_name, acc_list in global_test_acc_dict.items():
            writer.writerow([method_name] + acc_list)
