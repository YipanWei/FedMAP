...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'PROMPTFL_KL_Global',
                      "vision_depth": 0,
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.PROMPTFL_KL_Global.N_CTX
        ctx_init = cfg.TRAINER.PROMPTFL_KL_Global.CTX_INIT #   (XXXXX) class with the  sylte of XXXXXXX 16
        ...
        if ctx_init:
            ...
            if cfg.TRAINER.PROMPTFL_KL_Global.CSC:
        ...
        self.class_token_position = cfg.TRAINER.PROMPTFL_KL_Global.CLASS_TOKEN_POSITION

    def forward(self, ctx_override=None):
        # ctx_override: None 或 shape=[n_ctx, dim] 或 [n_cls, n_ctx, dim]
        ctx = self.ctx if ctx_override is None else ctx_override

        # 兼容 generic ctx: [n_ctx, dim] -> expand 到 [n_cls, n_ctx, dim]
        if ctx.dim() == 2:
...
class CustomCLIP(nn.Module):
    ...
    def _encode_text_with_ctx(self, ctx_override=None):
        prompts = self.prompt_learner(ctx_override=ctx_override)
        tokenized_prompts = self.tokenized_prompts
        text_features = self.text_encoder(prompts, tokenized_prompts)
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)
        return text_features

    def forward(self, image, global_ctx=None):
        # image_features
        ...
        # local
        local_text_features = self._encode_text_with_ctx(ctx_override=None)
        local_logits = logit_scale * image_features @ local_text_features.t()

        # global (optional)
        if global_ctx is not None:
            global_text_features = self._encode_text_with_ctx(ctx_override=global_ctx)
            global_logits = logit_scale * image_features @ global_text_features.t()
        else:
            global_text_features = None
            global_logits = None

        return local_logits, image_features, local_text_features, global_logits, global_text_features

class PROMPTFL_KL_Global(TrainerX):

    # =========================
    # Your original trainer methods below
    # =========================
    def check_cfg(self, cfg):
        assert cfg.TRAINER.PROMPTFL_KL_Global.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.PROMPTFL_KL_Global.PREC in ["fp32", "amp"]:
            clip_model.float()
        ...
        self.scaler = GradScaler() if cfg.TRAINER.PROMPTFL_KL_Global.PREC == "amp" else None

    def forward_backward(self, idx, batch_idx, batch, **kwargs):
        image, label = self.parse_batch_train(batch)
        prec = self.cfg.TRAINER.PROMPTFL_KL_Global.PREC

        T = float(getattr(self.cfg.TRAINER.PROMPTFL_KL_Global, "KD_T", 2.0))
        lam_kl = float(getattr(self.cfg.TRAINER.PROMPTFL_KL_Global, "LAM_KL", 0.5))

        global_ctx = None
        if "global_weights" in kwargs and "prompt_learner.ctx" in kwargs["global_weights"]:
            global_ctx = kwargs["global_weights"]["prompt_learner.ctx"]
            global_ctx = global_ctx.to(self.device).to(self.model.dtype)

        if prec == "amp":
            with autocast():
                local_logits, _, _, global_logits, _ = self.model(image, global_ctx=global_ctx)
                loss_ce = F.cross_entropy(local_logits, label)

                loss_kl = 0.0
                if global_logits is not None:
                    log_p_s = F.log_softmax(local_logits / T, dim=1)
                    p_t = F.softmax(global_logits / T, dim=1).detach()
                    loss_kl = F.kl_div(log_p_s, p_t, reduction="batchmean") * (T * T)

                loss = loss_ce + lam_kl * loss_kl
        ...
        loss_summary = {
            "loss": float(loss_ce.item()),
            "acc": compute_accuracy(local_logits, label)[0].item(),
            "kl": float(loss_kl) if not torch.is_tensor(loss_kl) else float(loss_kl.item()),
        }
    ...
    # ============================================================
    # ✅ Added: unified saving utilities (manifest + embeddings + ctx)
    # Keep old interfaces to avoid breaking external calls
    # ============================================================
    ...
    def _run_tag(self):
        ...
        # KL-global trainer specific tags
        T = float(getattr(cfg.TRAINER.PROMPTFL_KL_Global, "KD_T", 1.0))
        ...
        return f"seed{seed}_T{T:g}_lk{lam_kl:g}_prec{prec}"
