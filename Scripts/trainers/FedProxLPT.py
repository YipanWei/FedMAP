...
def load_clip_to_cpu(cfg, trainer):
    ...
    design_details = {"trainer": 'FedProxLPT',
                      "vision_depth": 0,
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.FedProxLPT.N_CTX_TEXT
        ctx_init = cfg.TRAINER.FedProxLPT.CTX_INIT
        ...
        if ctx_init and (n_ctx) <= 4:
            ...
            ctx_vectors = embedding[0, 1: 1 + n_ctx, :]  # 使用预先定义的文本进行优化
            prompt_prefix = ctx_init
        else:
            if cfg.TRAINER.FedProxLPT.CSC:
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        logits = logit_scale * norm_image_features @ norm_text_features.t()
        return logits, norm_image_features, norm_text_features, image_features, text_features


class FedProxLPT(TrainerX):
    def check_cfg(self, cfg):
        assert cfg.TRAINER.FedProxLPT.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        self.scaler = GradScaler() if cfg.TRAINER.FedProxLPT.PREC == "amp" else None

        self.global_text_features = None
        self.last_text_features = {}

    def forward_backward(self, idx, batch_idx, batch, **kwargs):
        image, label = self.parse_batch_train(batch)
        prec = self.cfg.TRAINER.FedProxLPT.PREC
        if prec == "amp":
            with autocast():
        ...
        if (self.batch_idx + 1) == self.num_batches:
            with torch.no_grad():
                self.last_text_features[idx] = self.model(image)[2]
            self.update_lr()
