...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'VPT',
                      "vision_depth": cfg.TRAINER.VPT_Ma_KL.PROMPT_DEPTH_VISION,
                      "language_depth": 0, "vision_ctx": cfg.TRAINER.VPT_Ma_KL.N_CTX_VISION,
                      "language_ctx": 0}
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.tau_s = 1.0      # student temperature
        self.tau_t = 2.0      # teacher temperature (通常更大，更平滑)
        self.eps_smooth = 0.05  # teacher label smoothing，缓解“语言拓扑不一定真值”的过强约束
    ...
    def forward(self, image,  label=None):
        ...
        # Part 3: 机制 B - KL 结构蒸馏
        # -----------------------------------------------------
        loss_struct = torch.tensor(0.0, device=image.device)

        if self.training and label is not None:
            # 1) 更新视觉原型（无梯度）
            self.update_visual_prototypes(image_features.detach(), label)

            # 2) 视觉关系：student logits  s_rel = sim(z, q_k)
            current_vis_relations = image_features.float() @ self.visual_prototypes.float().t()  # [B, C]

            # 3) 文本关系：teacher logits  t_rel = sim(a_y, a_k)
            batch_text_anchors = self.fixed_text_anchors[label]  # [B, D]
            target_text_relations = batch_text_anchors.float() @ self.fixed_text_anchors.float().t()  # [B, C]

            # 4) 只在 active 类子集上做 KL（避免 mask 破坏概率归一）
            active_idx = self.active_classes.nonzero(as_tuple=True)[0]  # [A]

            if active_idx.numel() >= 2:
                s_rel = current_vis_relations[:, active_idx]  # [B, A]
                t_rel = target_text_relations[:, active_idx]  # [B, A]

                # student: log-prob
                log_p_s = F.log_softmax(s_rel / self.tau_s, dim=1)

                # teacher: prob
                p_t = F.softmax(t_rel / self.tau_t, dim=1).detach()

                # optional: teacher smoothing (推荐开着，稳定很多)
                eps = self.eps_smooth
                if eps > 0:
                    A = p_t.size(1)
                    p_t = (1 - eps) * p_t + eps / A
                ...
                # KL(p_t || p_s)
                loss_struct = F.kl_div(log_p_s, p_t, reduction="batchmean")
            else:
                loss_struct = torch.tensor(0.0, device=image.device)
...
class VPT_Ma_KL(TrainerX):
    def check_cfg(self, cfg):
        assert cfg.TRAINER.VPT_Ma_KL.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.VPT_Ma_KL.PREC == "fp32" or cfg.TRAINER.VPT_Ma_KL.PREC == "amp":
            clip_model.float()
        ...
        self.scaler = GradScaler() if cfg.TRAINER.VPT_Ma_KL.PREC == "amp" else None

        # 定义结构损失的权重 (超参数)
        self.lambda_kl =cfg.TRAINER.VPT_Ma_KL.lambda_struct # 可以根据实验调整，通常设为 1.0 到 10.0

    def forward_backward(self, idx, batch_idx, batch, **kwargs):
        image, label = self.parse_batch_train(batch)
        prec = self.cfg.TRAINER.VPT_Ma_KL.PREC
        ...
        if prec == "amp":
            with autocast():
                ...
                loss = loss_cls + self.lambda_kl * loss_struct
