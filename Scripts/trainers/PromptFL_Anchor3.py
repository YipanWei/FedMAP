...
# where:
#   n_ctx = cfg.TRAINER.PROMPTFL_Anchor3.N_CTX
...
LAMBDA_DIVERSE = 1e-2  # try 1e-3 ~ 1e-2 as a safe range
...
class PromptLearner(nn.Module):
    ...
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = int(cfg.TRAINER.PROMPTFL_Anchor3.N_CTX)
        ...
        self.branch_gate = nn.Parameter(torch.zeros(self.anchor_num))
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, return_branch_logits=False):
        """
        returns:
          logits: (B, C)  (gated fusion)
          image_features: (B, D)
          text_features_acd: (A, C, D)
          (optional) logits_abc: (A, B, C)  for branch-wise eval/debug
          (optional) gate_w: (A,)           softmax weights
        """
        ...
        text_features_acd = text_features_flat.reshape(A, C, -1)
        ...
        # ---- gated fusion ----
        gate_w = F.softmax(self.prompt_learner.branch_gate, dim=0)  # (A,)
        logits = (gate_w[:, None, None] * logits_abc).sum(dim=0)    # (B, C)

        if return_branch_logits:
            return logits, image_features, text_features_acd, logits_abc, gate_w
        else:
            return logits, image_features, text_features_acd



class PROMPTFL_Anchor3(TrainerX):
    """
    Same name kept to align with your existing configs/usages,
    but internally upgraded to multi-branch + diversity loss.
    """
    def check_cfg(self, cfg):
        assert cfg.TRAINER.PROMPTFL_Anchor3.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.PROMPTFL_Anchor3.PREC in ["fp32", "amp"]:
            clip_model.float()
        ...
        print(f"[Diversity] LAMBDA_DIVERSE = {LAMBDA_DIVERSE}")
        ...
        self.scaler = GradScaler() if cfg.TRAINER.PROMPTFL_Anchor3.PREC == "amp" else None

    def forward_backward(self, idx, batch_idx, batch, **kwargs):
        image, label = self.parse_batch_train(batch)
        prec = self.cfg.TRAINER.PROMPTFL_Anchor3.PREC

        # 每隔一段打印 gate（你也可以改频率）
        print_gate = (self.epoch % 5 == 0) and (batch_idx == 0)

        if prec == "amp":
            with autocast():
                if print_gate:
                    logits, _, text_features_acd, logits_abc, gate_w = self.model(image, return_branch_logits=True)
                else:
                    logits, _, text_features_acd = self.model(image, return_branch_logits=False)

                loss_ce = F.cross_entropy(logits, label)
                loss_div = branch_diversity_loss(text_features_acd)
                loss = loss_ce + LAMBDA_DIVERSE * loss_div
        ...
        # ---- debug: print gate weights + per-branch train acc for this batch ----
        if print_gate:
            gw = gate_w.detach().cpu().tolist()
            print(f"[Gate] epoch={self.epoch} gate_w={['%.3f'%x for x in gw]} (sum={sum(gw):.3f})")

            # branch-wise acc on this batch（训练批次层面，只作趋势参考）
            with torch.no_grad():
                # logits_abc: (A,B,C)
                A = logits_abc.size(0)
                for a in range(A):
                    acc_a = compute_accuracy(logits_abc[a], label)[0].item()
                    print(f"[Gate] branch {a} batch_acc = {acc_a:.2f}")
    ...
    def after_train(self, idx=-1, epoch=0, is_fed=False):
        ...
        if idx >= 0 and epoch >= 0:
            with torch.no_grad():
                gw = F.softmax(self.model.prompt_learner.branch_gate, dim=0).cpu().tolist()
                print(f"[Gate][AfterLocal] user={idx} gw={['%.3f'%x for x in gw]}")
