...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'VPT',
                      "vision_depth": cfg.TRAINER.PROMPTFL_KL_VPT.PROMPT_DEPTH_VISION,
                      "language_depth": 0, "vision_ctx": cfg.TRAINER.PROMPTFL_KL_VPT.N_CTX_VISION,
                      "language_ctx": 0}
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.PROMPTFL_KL_VPT.N_CTX
        ctx_init = cfg.TRAINER.PROMPTFL_KL_VPT.CTX_INIT
        ...
        if ctx_init:
            ...
            if cfg.TRAINER.PROMPTFL_KL_VPT.CSC:
        ...
        self.class_token_position = cfg.TRAINER.PROMPTFL_KL_VPT.CLASS_TOKEN_POSITION
...
class PROMPTFL_KL_VPT(TrainerX):
    """
    Strict extraction of:
      PROMPTFL_KL_VPT with INJECT.MODE=kl, SEMANTIC.FORM=attr, SEMANTIC.USE_ALL=True,
      SEMANTIC.MULTI_REDUCE=mean_then_calc

    Goal: with same seed & same dataloader order, the loss/acc logits logs should match PROMPTFL_KL_VPT exactly.
    """

    def check_cfg(self, cfg):
        # keep identical precision constraint
        assert cfg.TRAINER.PROMPTFL_KL_VPT.PREC in ["fp16", "fp32", "amp"]
    # -------------------------
    # LLM embedding loader (KL + attr only)
    # -------------------------
    def _load_llm_attr_embeddings_kl(self):
        """
        This is a strict subset of PROMPTFL_KL_VPT._load_llm_expansion()
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.PROMPTFL_KL_VPT.PREC in ["fp32", "amp"]:
            clip_model.float()
        ...
        self.scaler = GradScaler() if cfg.TRAINER.PROMPTFL_KL_VPT.PREC == "amp" else None

        # ONLY load KL attr embeddings
        self._load_llm_attr_embeddings_kl()

        print("=" * 90)
        print("[RUN] dataset=", cfg.DATASET.NAME, "| trainer=", type(self).__name__, "| nctx=", cfg.TRAINER.PROMPTFL_KL_VPT.N_CTX)
    ...
    # -------------------------
    # exact KL pieces (copied from PROMPTFL_KL_VPT branch)
    # -------------------------
    def forward_backward(self, idx, batch_idx, batch, **kwargs):
        cfg = self.cfg
        prec = cfg.TRAINER.PROMPTFL_KL_VPT.PREC
        ...
        def _compute_losses_kl_mean_then_calc(model_out):
            """
            EXACT behavior of PROMPTFL_KL_VPT:
    ...
    def load_model(self, directory, epoch=None):
        # keep identical to PROMPTFL_KL_VPT (you can reuse yours verbatim)
        if not directory:
