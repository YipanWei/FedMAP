...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'VPT',
                      "vision_depth": cfg.TRAINER.VPT_Ma_R.PROMPT_DEPTH_VISION,
                      "language_depth": 0, "vision_ctx": cfg.TRAINER.VPT_Ma_R.N_CTX_VISION,
                      "language_ctx": 0}
...
class VPT_Ma_R(TrainerX):
    def check_cfg(self, cfg):
        assert cfg.TRAINER.VPT_Ma_R.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.VPT_Ma_R.PREC == "fp32" or cfg.TRAINER.VPT_Ma_R.PREC == "amp":
            clip_model.float()
        ...
        # ============================================================
        # 【方案二】直接使用 Random 值初始化 (不读取任何文件)
        # ============================================================

        target_classnames = self.dm.dataset.classnames
        num_classes = len(target_classnames)

        # 获取特征维度 (需确保 self.model 已经初始化)
        # 通常 CLIP 的 visual output dim 也就是 text projection dim
        feat_dim = self.model.feat_dim

        print(f"Generating RANDOM embeddings from Seed: {self.cfg.SEED}")
        ...
        # 生成随机高斯噪声
        # 形状说明: [Num_Classes, 1, Dim]
        # 中间保留 "1" 这个维度是为了模拟 "1个模板" 的格式，
        # 这样 self.model.init_text_topology 里的 .mean(dim=1) 计算依然成立且正确
        self.class_embeddings = torch.randn(
            num_classes, 1, feat_dim, generator=rng
        ).to(self.device)

        print(f"✅ Random embeddings generated. Shape: {self.class_embeddings.shape}")

        # 初始化拓扑 (init_text_topology 会处理归一化，所以这里直接传 randn 没问题)
        ...
        self.scaler = GradScaler() if cfg.TRAINER.VPT_Ma_R.PREC == "amp" else None

        # 定义结构损失的权重 (超参数)
        self.lambda_struct =cfg.TRAINER.VPT_Ma_R.lambda_struct # 可以根据实验调整，通常设为 1.0 到 10.0

    def forward_backward(self, idx, batch_idx, batch, **kwargs):
        image, label = self.parse_batch_train(batch)
        prec = self.cfg.TRAINER.VPT_Ma_R.PREC
