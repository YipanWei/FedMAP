...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'VPT',
                      "vision_depth": cfg.TRAINER.VPT_Ma_A.PROMPT_DEPTH_VISION,
                      "language_depth": 0, "vision_ctx": cfg.TRAINER.VPT_Ma_A.N_CTX_VISION,
                      "language_ctx": 0}
...
class VPT_Ma_A(TrainerX):
    def check_cfg(self, cfg):
        assert cfg.TRAINER.VPT_Ma_A.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.VPT_Ma_A.PREC == "fp32" or cfg.TRAINER.VPT_Ma_A.PREC == "amp":
            clip_model.float()
        ...
        # ===========================
        # [NEW] Text embedding switch
        # ===========================
        emb_source = getattr(self.cfg.TRAINER.VPT_Ma_A, "TEXT_EMB_SOURCE", "class_attr")
        emb_source = emb_source.lower()

        # 这四种就是 A/B/C/D
        SUFFIX_MAP = {
            "class_attr": "",               # A: {class} : {attribute}
            "cls_photo": "_cls_photo",      # B: a photo of a {cls}.
            "cls_only": "_cls_only",        # C: {cls}
            "attr_only": "_attr_only",      # D: {attribute}
        }

        if emb_source not in SUFFIX_MAP:
            raise ValueError(
                f"Unknown TEXT_EMB_SOURCE='{emb_source}'. "
                f"Valid: {list(SUFFIX_MAP.keys())}"
            )

        suffix = SUFFIX_MAP[emb_source]
        embedding_path = f"./embeddings/{dataset_name}/text/{prefix}_template_attributes{suffix}.pt"

        if osp.exists(embedding_path):
            print(f"Loading text embeddings ({emb_source}) from: {embedding_path}")

            # torch.load 兼容一下不同 torch 版本
            try:
                emb_data = torch.load(embedding_path, weights_only=True)
            except TypeError:
                emb_data = torch.load(embedding_path)

            raw_dict = emb_data["embeddings"]  # dict: {class_name: Tensor}
            ...
            for cls_name in target_classnames:
                if cls_name not in raw_dict:
                    raise ValueError(
                        f"Class '{cls_name}' not found in embedding dict keys. "
                        f"Keys example: {list(raw_dict.keys())[:10]}"
                    )

                t = raw_dict[cls_name]  # 可能是 [D] 或 [K,D]

                # 统一成 [K, D]
                if t.dim() == 1:
                    t = t.unsqueeze(0)  # [1, D]  (对应 B/C)
                elif t.dim() == 2:
                    pass                # [K, D]  (对应 A/D)
                else:
                    raise ValueError(f"Unexpected embedding tensor shape for class '{cls_name}': {tuple(t.shape)}")

                embedding_list.append(t)

            # 统一堆叠成 [C, K, D]
            self.class_embeddings = torch.stack(embedding_list, dim=0).to(self.device)

            print(f"✅ Text embeddings loaded. Source={emb_source}, shape={self.class_embeddings.shape}")

            # 初始化文本拓扑/锚点（机制 A/B 的基础）
            self.model.init_text_topology(self.class_embeddings)

        else:
            raise FileNotFoundError(
                f"Text embedding file not found: {embedding_path}\n"
                f"Check dataset_name={dataset_name}, prefix={prefix}, TEXT_EMB_SOURCE={emb_source}"
            )
        ...
        self.scaler = GradScaler() if cfg.TRAINER.VPT_Ma_A.PREC == "amp" else None

        # 定义结构损失的权重 (超参数)
        self.lambda_struct =cfg.TRAINER.VPT_Ma_A.lambda_struct # 可以根据实验调整，通常设为 1.0 到 10.0

    def forward_backward(self, idx, batch_idx, batch, **kwargs):
        image, label = self.parse_batch_train(batch)
        prec = self.cfg.TRAINER.VPT_Ma_A.PREC
