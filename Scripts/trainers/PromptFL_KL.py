...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'PROMPTFL_KL',
                      "vision_depth": 0,
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.PROMPTFL_KL.N_CTX
        ctx_init = cfg.TRAINER.PROMPTFL_KL.CTX_INIT
        ...
        if ctx_init:
            ...
            if cfg.TRAINER.PROMPTFL_KL.CSC:
        ...
        self.class_token_position = cfg.TRAINER.PROMPTFL_KL.CLASS_TOKEN_POSITION
...
class PROMPTFL_KL(TrainerX):
    """
    Strict extraction of:
      PROMPTFL_KL with INJECT.MODE=kl, SEMANTIC.FORM=attr, SEMANTIC.USE_ALL=True,
      SEMANTIC.MULTI_REDUCE=mean_then_calc

    Goal: with same seed & same dataloader order, the loss/acc logits logs should match PROMPTFL_KL exactly.
    """

    def check_cfg(self, cfg):
        # keep identical precision constraint
        assert cfg.TRAINER.PROMPTFL_KL.PREC in ["fp16", "fp32", "amp"]
    # -------------------------
    # LLM embedding loader (KL + attr only)
    # -------------------------
    def _load_llm_attr_embeddings_kl(self):
        """
        This is a strict subset of PROMPTFL_KL._load_llm_expansion()
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.PROMPTFL_KL.PREC in ["fp32", "amp"]:
            clip_model.float()
        ...
        self.scaler = GradScaler() if cfg.TRAINER.PROMPTFL_KL.PREC == "amp" else None

        # ONLY load KL attr embeddings
        self._load_llm_attr_embeddings_kl()

        print("=" * 90)
        print("[RUN] dataset=", cfg.DATASET.NAME, "| trainer=", type(self).__name__, "| nctx=", cfg.TRAINER.PROMPTFL_KL.N_CTX)
        print("[RUN] inject=kl | form=attr | use_all=True | multi_reduce=mean_then_calc")
        print("[RUN] lambda_kl=", cfg.TRAINER.PROMPTFL_KL.LAM_KL, "| T=", cfg.TRAINER.PROMPTFL_KL.KD_T)
        print("[RUN][EMB] embedding=", tuple(self.embedding.shape),
              "mask_valid=", int(self.embedding_mask.sum().item()))
        print("=" * 90)

    # -------------------------
    # exact KL pieces (copied from PROMPTFL_KL branch)
    # -------------------------
    def forward_backward(self, idx, batch_idx, batch, **kwargs):
        cfg = self.cfg
        prec = cfg.TRAINER.PROMPTFL_KL.PREC

        image, label = self.parse_batch_train(batch)

        def _kl_student_teacher(logits_student, logits_teacher):
            T = float(getattr(cfg.TRAINER.PROMPTFL_KL, "KD_T", 2.0))
        ...
        def _compute_losses_kl_mean_then_calc(model_out):
            """
            EXACT behavior of PROMPTFL_KL:
              use_all=True but multi_reduce == mean_then_calc
              -> proto mean -> logits_teacher -> KL
            """
            logits_prompt, image_features, _ = model_out
            loss_ce = F.cross_entropy(logits_prompt, label)
            lam = float(getattr(cfg.TRAINER.PROMPTFL_KL, "LAM_KL", 0.5))
            ...
            return loss_total, loss_ce, loss_kl, logits_prompt
    ...
    def load_model(self, directory, epoch=None):
        # keep identical to PROMPTFL_KL (you can reuse yours verbatim)
        if not directory:
    ...
    def _run_tag(self):
        ...
        T = float(getattr(cfg.TRAINER.PROMPTFL_KL, "KD_T", 1.0))
        lam_kl = float(getattr(cfg.TRAINER.PROMPTFL_KL, "LAM_KL", 1.0))
        prec = str(getattr(cfg.TRAINER.PROMPTFL_KL, "PREC", "unknown"))
    ...
    def _get_prompt_embed_run_dir(self):
        ...
        nctx = None
        try:
            nctx = int(getattr(cfg.TRAINER.PROMPTFL_KL, "N_CTX"))
        except Exception:
    ...
    def _maybe_dump_manifest(self):
        ...
        payload = {
            ...
            "kd_T": float(getattr(cfg.TRAINER.PROMPTFL_KL, "KD_T", 1.0)),
            "lam_kl": float(getattr(cfg.TRAINER.PROMPTFL_KL, "LAM_KL", 1.0)),
            "prec": str(getattr(cfg.TRAINER.PROMPTFL_KL, "PREC", "unknown")),
