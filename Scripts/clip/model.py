...
class VisionTransformer_fedmvp(nn.Module):
    """
    FedMVP-ready VisionTransformer:
      - 支持动态注入 prompts P（来自 PromptFormer）
      - forward() 兼容原来的返回：x_cls, x_patch
      - 额外提供 forward_with_prompts()：I=[z;E;P] -> 走 transformer

    Notes:
      - P 的维度应为 [B, m, width]（注入在 ln_pre 之前，和 token 同维度）
      - positional_embedding 仍只覆盖 [CLS + patches]，P 的 positional 由 PromptFormer 自己处理（推荐）
    """
    ...
    def _patchify(self, x: torch.Tensor) -> torch.Tensor:
        """
        x -> patch tokens (no CLS), with shape [B, b, width]
        """
        x = self.conv1(x)                                # [B, width, gh, gw]
        x = x.reshape(x.shape[0], x.shape[1], -1)        # [B, width, b]
        x = x.permute(0, 2, 1)                           # [B, b, width]
        return x

    def _add_cls_and_pos(self, patches: torch.Tensor) -> torch.Tensor:
        """
        patches: [B, b, width] -> tokens [B, 1+b, width] with positional embedding (for CLS+patches only)
        """
        B, b, w = patches.shape
        cls = self.class_embedding.to(patches.dtype) + torch.zeros(
            B, 1, w, dtype=patches.dtype, device=patches.device
        )
        x = torch.cat([cls, patches], dim=1)             # [B, 1+b, width]
        x = x + self.positional_embedding.to(patches.dtype)
        return x

    def get_z_E(self, image: torch.Tensor):
        """
        Return:
          z: [B, 1, width]  (CLS token with pos)
          E: [B, b, width]  (patch tokens with pos)
        """
        patches = self._patchify(image)                  # [B, b, width]
        x = self._add_cls_and_pos(patches)               # [B, 1+b, width]
        z = x[:, :1, :]
        E = x[:, 1:1 + self.num_patches, :]
        return z, E

    # ----------------------------
    # forward with dynamic prompts
    # ----------------------------
    def forward_with_prompts(self, z: torch.Tensor, E: torch.Tensor, P: torch.Tensor = None):
        """
        z: [B,1,width]
        E: [B,b,width]
        P: [B,m,width] or None (FedMVP dynamic prompts)

        Return:
          x_cls:  [B, output_dim]
          x_path: [B, (b + m + vpt_ctx), output_dim]   # 不含 CLS
        """
        x = torch.cat([z, E], dim=1)  # [B, 1+b, width]

        # 动态 prompts 注入（FedMVP：I=[z;E;P]）
        if P is not None:
            P = P.to(dtype=x.dtype, device=x.device)
            x = torch.cat([x, P], dim=1)  # [B, 1+b+m, width]

        # 兼容：如果你仍然想叠加 VPT shallow
        if self.VPT_shallow:
            visual_ctx = self.VPT.expand(x.shape[0], -1, -1).to(dtype=x.dtype, device=x.device)
            x = torch.cat([x, visual_ctx], dim=1)
        else:
            assert self.prompt_till_layer_visual == 0
    ...
    # ----------------------------
    # default forward (no dynamic prompts)
    # ----------------------------
    def forward(self, x: torch.Tensor):
        z, E = self.get_z_E(x)
        return self.forward_with_prompts(z, E, P=None)
    ...
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)): 
            ...
            if trainer == "GLP_OT":
                ...
                self.visual = VisionTransformer_fedmvp(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
